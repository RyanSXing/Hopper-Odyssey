document.addEventListener('DOMContentLoaded', () => {
    ...
    updatePlayerPosition()
    const completedQuestNaToSa = localStorage.getItem("completedQuestNaToSa");
    if (completedQuestNaToSa === "1") {
        moveAlongPath("na-to-sa");
    }
    const completedQuestSaToAf = localStorage.getItem("completedQuestSaToAf");
    if(completedQuestSaToAf === "1") {
        moveAlongPath("sa-to-af")
    }
    const completedQuestAfToEu = localStorage.getItem("completedQuestAfToEu");
    if(completedQuestAfToEu === "1") {
        moveAlongPath("af-to-eu")
    }
    const completedQuestEuToAs = localStorage.getItem("completedQuestEuToAs");
    if(completedQuestEuToAs === "1") {
        moveAlongPath("eu-to-as")
    }
    const completedQuestAsToAus = localStorage.getItem("completedQuestAsToAus");
    if(completedQuestAsToAus === "1") {
        moveAlongPath("as-to-aus")
        localStorage.clear();
    }
